...
import { BookOpen, Calendar, Star, Bell, Target, TrendingUp, Sparkles } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [suggestedMentors, setSuggestedMentors] = useState<(User & { user_skills?: any[] })[]>([]);
  ...
  const getGoalStatusColor = (status: string) => {
    switch (status) {
      case 'wanted': return 'rgba(255, 120, 64, 0.2)';
      case 'learning': return 'rgba(62, 207, 142, 0.2)';
      case 'completed': return 'rgba(120, 64, 255, 0.2)';
  ...
  const getGoalStatusBorder = (status: string) => {
    switch (status) {
      case 'wanted': return 'rgba(255, 120, 64, 0.3)';
      case 'learning': return 'rgba(62, 207, 142, 0.3)';
      case 'completed': return 'rgba(120, 64, 255, 0.3)';
      default: return 'rgba(255,255,255,0.1)';
    }
  };
      ...
      <div className="space-y-8">
        <div className="relative">
          ...
          <div className="relative z-10">
            <h1 className="text-5xl font-normal mb-3" style={{ letterSpacing: '-0.03em' }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          <div className="glass-card p-5 col-span-2 sm:col-span-1 lg:col-span-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>My Skills</p>
                <p className="text-2xl gradient-text font-medium">{stats?.total_skills || 0}</p>
              </div>
            </div>
          </div>
          <div className="glass-card p-5 col-span-2 sm:col-span-1 lg:col-span-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>Sessions</p>
                <p className="text-2xl font-medium" style={{ color: '#b4a0ff' }}>{stats?.upcoming_sessions || 0}</p>
              </div>
            </div>
          </div>
          <div className="glass-card p-5 col-span-2 sm:col-span-1 lg:col-span-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-medium" style={{ color: '#40e0e0' }}>{stats?.avg_rating ? Number(stats.avg_rating).toFixed(1) : '-'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              <div className="p-3 rounded-xl relative" style={{ background: 'linear-gradient(135deg, rgba(255, 64, 96, 0.2) 0%, rgba(255, 64, 96, 0.05) 100%)' }}>
                ...
                {(stats?.unread_notifications || 0) > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-xs flex items-center justify-center" style={{ background: '#ff4060', color: 'white' }}>
                    {(stats?.unread_notifications || 0) > 9 ? '9+' : stats?.unread_notifications}
                  </span>
                )}
              </div>
              <div>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>Alerts</p>
                <p className="text-2xl font-medium" style={{ color: '#ff4060' }}>{stats?.unread_notifications || 0}</p>
              </div>
            </div>
          </div>
          <div className="glass-card p-5 col-span-2 sm:col-span-1 lg:col-span-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>Goals</p>
                <p className="text-2xl gradient-text font-medium">{learningGoals.length}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             <div className="lg:col-span-1 glass-card p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/skills" className="text-sm" style={{ color: '#3ecf8e' }}>+ Add</Link>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs px-2 py-1 rounded-full capitalize" style={{ 
                        ...
                        color: goal.status === 'wanted' ? '#ff7840' : goal.status === 'learning' ? '#3ecf8e' : '#7840ff'
                      }}>
                        {goal.status}
                      </span>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
           <div className="glass-card p-5">
            ...
            {notifications.length > 0 ? (
              ...
              <p className="text-center py-8" style={{ color: 'rgba(255,255,255,0.4)' }}>No notifications yet</p>
            )}
          </div>
           ...
           <div className="glass-card p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link
                ...
                style={{ background: 'linear-gradient(135deg, rgba(62, 207, 142, 0.1) 0%, rgba(62, 207, 142, 0.03) 100%)', border: '1px solid rgba(62, 207, 142, 0.2)' }}
              >
                <BookOpen size={24} style={{ color: '#3ecf8e' }} className="mb-2" />
                <h3 className="font-medium">My Skills</h3>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>Manage expertise</p>
              </Link>
              <Link
                to="/browse"
                className="block p-5 rounded-xl transition-all duration-300 hover:scale-[1.02]"
                style={{ background: 'linear-gradient(135deg, rgba(120, 64, 255, 0.1) 0%, rgba(120, 64, 255, 0.03) 100%)', border: '1px solid rgba(120, 64, 255, 0.2)' }}
              >
                <Target size={24} style={{ color: '#7840ff' }} className="mb-2" />
                <h3 className="font-medium">Find Mentors</h3>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>Browse experts</p>
              </Link>
              <Link
                to="/sessions"
                className="block p-5 rounded-xl transition-all duration-300 hover:scale-[1.02]"
                style={{ background: 'linear-gradient(135deg, rgba(255, 120, 64, 0.1) 0%, rgba(255, 120, 64, 0.03) 100%)', border: '1px solid rgba(255, 120, 64, 0.2)' }}
              >
                <Calendar size={24} style={{ color: '#ff7840' }} className="mb-2" />
                <h3 className="font-medium">Sessions</h3>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>View bookings</p>
              </Link>
              <Link
                to="/leaderboard"
                className="block p-5 rounded-xl transition-all duration-300 hover:scale-[1.02]"
                style={{ background: 'linear-gradient(135deg, rgba(64, 224, 224, 0.1) 0%, rgba(64, 224, 224, 0.03) 100%)', border: '1px solid rgba(64, 224, 224, 0.2)' }}
              >
                <Star size={24} style={{ color: '#40e0e0' }} className="mb-2" />
                <h3 className="font-medium">Leaderboard</h3>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>Top mentors</p>
              </Link>
            </div>
          </div>
        </div>
